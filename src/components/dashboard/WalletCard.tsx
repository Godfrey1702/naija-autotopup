...
export function WalletCard({ balance, onAddFunds }: WalletCardProps) {
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-2xl gradient-primary p-4 shadow-glow"
    >
      {/* Background pattern */}
      <div className="absolute inset-0 opacity-20">
        <div className="absolute top-0 right-0 w-32 h-32 rounded-full bg-foreground/10 -translate-y-1/2 translate-x-1/2" />
        <div className="absolute bottom-0 left-0 w-24 h-24 rounded-full bg-foreground/10 translate-y-1/2 -translate-x-1/2" />
      </div>

      <div className="relative z-10 flex items-center justify-between">
        <div className="flex-1">
          <p className="text-primary-foreground/70 text-xs font-medium">Wallet Balance</p>
